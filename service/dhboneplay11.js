...
const  runRpc = async (username) => {
    ...
    //  获取所有的交易信息
        ...
        // let producer_block_id = actionHistory.actions[i].producer_block_id
         ...
         if (quantity!==""&&from!=="undefined"&&from!=="dhboneplay11"&&to==="dhboneplay11"){
             if(!res){
...
//         runRpc("betdiceadmin").catch(result=>{
//             //get info
//             console.log(result)
//         })
//     });
// }
//
// scheduleCronstyle().catch(result=>{
//     console.log(result)
// });


//定时任务 topdivacc111   三张牌账户 toppaigow123

const  myfunc=async (Interval)=>{
        runRpc("dhboneplay11").catch(result=>{
            //get info
            console.log("异常情况"+result)
        })
}

setInterval(myfunc,1010,"Interval");

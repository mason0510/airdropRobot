...
//开始游戏
async function startgame(accountname, privatekey) {
...
let bet=async (betassets,randomaccount,accountname,privatekey)=>{
    ...
    await request.post('https://proxy.eosnode.tools/v1/chain/get_table_rows', {
        ...
        console.log(gamestatus+"======"+roundId)
        if(gamestatus===2){
            //获取上一局输赢  上一局赢了  上一局输了新的玩家
            if(body.rows[0].largest_winner===accountname){
                await startgame(largest_winner, privatekey);
            }else {
                await startgame(accountname, privatekey);
            }
        }
    })
}

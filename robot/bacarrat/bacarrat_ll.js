...
start = async (arrInternal) => {
    ...
    for (let i = 0; i < allres.length; i++) {
        newarr.push(allres[i].accountname);
    }
     // newarr = allres;
     // console.log(newarr);
    ...
    if (status !== 2) {return console.log("游戏不活跃");}
        ...
        let resnumber = await randomNumber.norepeatNumber(5);
    ...
    console.log("accountname2====================================="+accountname4);
        ...
        for (let i = 0; i < allres.length; i++) {
            ...
            if (accountname1 === allres[i].accountname) {
                privatekey1 = await allres[i].privatekey;
            }
            if (accountname2 === allres[i].accountname) {
                privatekey2 = await allres[i].privatekey;
            }
            if (accountname3 === allres[i].accountname) {
                privatekey3 = await allres[i].privatekey;
            }
            if (accountname4 === allres[i].accountname) {
                privatekey4 = await allres[i].privatekey;
            }
        }
    ...
    if (verifyres==="0"){
        promise = new Promise(async (resolve, reject) => {
            ...
            if (await Internal.get_verify()==="true") {
                ...
                let memo1 = roundId + "," + newarr[resnumber] + "," + area1 + "," + robot_amount2.toString() + ",";
                gettxInfo = await PushAcition._betBaccarat(accountname1, privatekey1, quantity2, memo1, area1, roundId, end_time, playerInfos, gameTable).then(async (res) => {

                    if (res===undefined){
                        await PushAcition._betBaccarat(accountname1, privatekey1, quantity2, memo1 ,area1, roundId, end_time, playerInfos, gameTable)
                    }else {
                        bet_result = res;
                        resolve(bet_result);
                        //console.log("===============下注结果" + res);

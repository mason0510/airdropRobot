...
let eoshelper_copy=require("../../utils/eoshelper_copy");
    ...
    if (status===2) {
        if (bettime>=2) {
            ...
            try {
                ...
                await eoshelper_copy.api.myFunc(mykey).transact({
                    actions: [{
                        account: "eosio.token",
                        name: 'transfer',
                        authorization: [{
            ...
            let str="当前第" + count + "次下注" + "finish" + allAmount;
            fs.writeFile("../betsInfo",str,function (err) {
                if (err){
                    return;
                }
            })
...
let start=async ()=> {
    ...
    if (gameTable==null||playerInfos==null){
        return
    }
    ...
    for (let i = 0; i < res.length; i++) {
        ...
        let myprivatekeyPromise =  CryptoUtil.privateDecrypt(key);
        //必须串行

...
start=async ()=> {
    ...
    try {
       ...
       // let resnumber = await randomNumber.norepeatNumber(1);
        let accountname0 = res[0].accountname;
        let privatekey0 = res[0].privatekey;
        ...
        if (status === 2) {
            ...
            if (roundId % 2 === 0) {
                ...
                _bet(accountname0, privatekey0, "50000 EOS", memo1, area1, roundId, end_time, playerInfos, gameTable);
            } else {
                let area1 = await constants.betarea[Math.floor(Math.random() * constants.betarea.length)]

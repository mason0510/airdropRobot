...
start=async ()=> {
    ...
    try {
       ...
       // let resnumber = await randomNumber.norepeatNumber(1);
        let accountname0 = res[8].accountname;
        let privatekey0 = res[8].privatekey;
        ...
        if (status === 2) {
            if (verify)
            {
                ...
                if (roundId%4===0){
                    ...
                    let memo0 = roundId + "," + res[0].accountname + "," + area0 + "," + 10000 + ",";
                    _bet(accountname0, privatekey0, "1.0000 EOS", memo0, area0, roundId, end_time, playerInfos, gameTable);

                }else if (roundId%4===1) {
                    let area1 = await constants.betarea[Math.floor(Math.random() * constants.betarea.length)]
                    let memo1 = roundId + "," + res[0].accountname + "," + area1 + "," + 5000 + ",";
                    _bet(accountname0, privatekey0, "0.5000 EOS", memo1, area1, roundId, end_time, playerInfos, gameTable);
                }else if (roundId%4===2) {
                    let area1 = await constants.betarea[Math.floor(Math.random() * constants.betarea.length)]
                    let memo1 = roundId + "," + res[0].accountname + "," + area1 + "," + 500000 + ",";
                    _bet(accountname0, privatekey0, "50.0000 EOS", memo1, area1, roundId, end_time, playerInfos, gameTable);
                }else if (roundId%4===3) {
                    let area1 = await constants.betarea[Math.floor(Math.random() * constants.betarea.length)]
                    let memo1 = roundId + "," + res[0].accountname + "," + area1 + "," + 100000 + ",";
                    _bet(accountname0, privatekey0, "10.0000 EOS", memo1, area1, roundId, end_time, playerInfos, gameTable);
                }else{
                    let area1 = await constants.betarea[Math.floor(Math.random() * constants.betarea.length)]
                    let memo1 = roundId + "," + res[0].accountname + "," + area1 + "," + 50000 + ",";
                    _bet(accountname0, privatekey0, "5.0000 EOS", memo1, area1, roundId, end_time, playerInfos, gameTable);
                }
            }
            //重置数据
            verify = false;
